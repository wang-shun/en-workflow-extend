...
//	private IdentityService identityService;
	 ...
	 * @throws Exception 
	 */
	public Object goAnyWhere(HttpServletRequest request, HttpServletResponse response) throws Exception{
		WfResult wfresult = null;
		/*candidateUsers candidateGroup assignee*/
		String assignType = null;
		String assignValue = null;
		/*http get方法过来这样能够得到参数*/
		...
		String destTaskDefinitionKey = null;
		if(map!=null&&map.size()>0){
			entity = ((String[]) map.get("entity"))[0];
			isStart = ((String[]) map.get("isStart"))[0];
			formId = ((String[]) map.get("formId"))[0];
			wfOperatorStr = ((String[]) map.get("wfOperator"))[0];	
			bussinessKey = ((String[]) map.get("businessKey"))[0];
			currenTaskId = ((String[]) map.get("currenTaskId"))[0];
			destTaskDefinitionKey = ((String[]) map.get("destTaskDefinitionKey"))[0];
//			String taskDefKey = ((String[]) map.get("taskDefKey"))[0]; 
			processDefinitionId = ((String[]) map.get("processDefinitionId"))[0];
			variablesStr = ((String[]) map.get("variables"))[0];
			opinion = ((String[]) map.get("opinion"))[0];
			proInsId = ((String[]) map.get("proInsId"))[0];		
			transitionId = ((String[]) map.get("transitionId"))[0];	
		}
		if(map==null||map.size()==0){
		...
		if(variables.get(WorkFlowService.TYPE_ASSIGNEE)!=null){ //分派人
			...
			assignType = WorkFlowService.TYPE_ASSIGNEE;
			assignValue = s;
		...
		}else if(variables.get("candidateUsers")!=null){ //候选人
...
/*				WfResult wfs = new WfResult();
			...
			assignType = WorkFlowService.TYPE_CANDIDATEUSERS;
			assignValue = s;
			candidateUsers = s.substring(0, s.length()-1);
			variables.put("candidateUsers",candidateUsers);		
			assigneList.addAll(Arrays.asList((s.substring(0, s.length()-1)).split(",")));
			
		}else if(variables.get("candidateGroups")!=null){ //候选组
			assignType = WorkFlowService.TYPE_CANDIDATEGROUPS;
			assignValue = (String) variables.get("candidateGroups");
			...
			candidateUsers = sb.substring(0, sb.length()-1);
			variables.put("candidateUsers",candidateUsers);	
...
//			formService.updateFormDataWithExternalTable(bussinessKey,proInsId, entity, form);
		...
		variables.put("assigneeList", assigneList);
		/*更新一些关键流程变量*/
		if(entitymap.get(WorkFlowService.PRIKEY)!=null){
			variables.put(WorkFlowService.PRIKEY, entitymap.get(WorkFlowService.PRIKEY));
		}
		if(entitymap.get(WorkFlowService.WORKTITLEKEY)!=null){
			variables.put(WorkFlowService.WORKTITLEKEY, entitymap.get(WorkFlowService.WORKTITLEKEY));
		}	
		if(entitymap.get(WorkFlowService.APPLYBRANCHKEY)!=null){
			variables.put(WorkFlowService.APPLYBRANCHKEY, entitymap.get(WorkFlowService.APPLYBRANCHKEY));
		}	
		...
		wfresult =  wp.goAnyWhere(wfOperator, false, bussinessKey, processDefinitionId, currenTaskId, destTaskDefinitionKey, true, variables);
		/*通知处理*/
	...
	public List<WorkFlowTransition> getOutTransition(String procInstanceId,String procDefId,String activitiId){
		...
		for(ActivityImpl activityImpl:activitiList){  
			...
			if(activityImpl.getActivityBehavior() instanceof SubProcessActivityBehavior){
				List<WorkFlowTransition> subResult =  getOutTransitionInsub(activityImpl,activitiId);
...
//				System.out.println("当前任务ﺿ+activityImpl.getProperty("name")); //输出某个节点的某种属徿 
				List<PvmTransition> outTransitions = activityImpl.getOutgoingTransitions();//获取从某个节点出来的懿线路  
//				Execution eq = runtimeService.createExecutionQuery().activityId("a").singleResult();
//				historyService.createHistoricTaskInstanceQuery().processInstanceBusinessKey(arg0)
				...
				for(PvmTransition tr:outTransitions){  
					...
					map1.put("multiInstance",activityImpl.getProperty("multiInstance"));
...
//				historyService.createHistoricTaskInstanceQuery().processInstanceBusinessKey(arg0)
	...
	public PvmTransition getPvmTransitionById(String procDefId,String taskDefKey,String transitionId){
		...
		for(ActivityImpl activityImpl:activitiList){ 
			//是否子流程
			if(activityImpl.getActivityBehavior() instanceof SubProcessActivityBehavior){
				PvmTransition pvmTransition =  getPvmTransitionByIdInsub(activityImpl,taskDefKey,transitionId);
	...
	public PvmTransition getPvmTransitionByIdInsub(ActivityImpl activity,String taskDefKey,String transitionId){
//		repositoryService = ApplicationContextManager.getContext().getBean(RepositoryService.class);	  
//		ProcessDefinitionEntity def = (ProcessDefinitionEntity) ((RepositoryServiceImpl)repositoryService).getDeployedProcessDefinition(procDefId);  	  
	 ...
	 * @return
	 */
	public List<DropDownListOption> getActivityCandidates(String processDefinitionId,String moduleId,String taskDefKey){
		Map map = new HashMap();
		...
		list = sqlsession.selectList("com.chinacreator.c2.omp.workflowcommon.WorkFlowMapper.getActivityCandidates",map);
		for(Map map1:list){
			...
			if(performer!=null&&!performer.isEmpty()){
				...
				for(String s:performers){
					...
					List<Map> list1 = sqlsession.selectList("com.chinacreator.c2.omp.workflowcommon.WorkFlowMapper.getUserInfoById",map1);
			...
			if(group_performer!=null&&!group_performer.isEmpty()){
				...
				for(String s:groups){
					...
					List<Map> list1 = sqlsession.selectList("com.chinacreator.c2.omp.workflowcommon.WorkFlowMapper.getGroupInfoById",map1);
 ...
 * @throws Exception
 */
	public List<WorkFlowComment> getAllHistoricProcessInstanceComments(String proinsid) throws Exception{
		if(wfHistoryService==null){
			wfHistoryService = ApplicationContextManager.getContext().getBean(WfHistoryService.class);
		}
		if(userService==null){
		...
		historyService = ApplicationContextManager.getContext().getBean(HistoryService.class);
		List<WorkFlowComment> list =new ArrayList<WorkFlowComment>();
//		List<WfComment> wfcs = wfHistoryService.getAllHistoricProcessInstanceComments(proinsid);
		...
		for(Comment wfc:coms){
			...
			HistoricTaskInstance histask = historyService.createHistoricTaskInstanceQuery().taskId(wfc.getTaskId()).singleResult();
	...
	public int getTodoWorkTotalBySP(ServiceProduct sp,String userId){
		...
		if(userId!=null&&productNo!=null){
			l = taskService.createTaskQuery().processVariableValueEquals(WorkFlowService.PRODUCTNO,productNo).taskCandidateOrAssigned(userId).processDefinitionKeyLike(proccessDefKey).count();
		}else if(userId!=null&&productNo==null){
			l = taskService.createTaskQuery().taskCandidateOrAssigned(userId).processDefinitionKeyLike(proccessDefKey).count();
		}else if(userId==null&&productNo!=null){
			l = taskService.createTaskQuery().processVariableValueEquals(WorkFlowService.PRODUCTNO,productNo).processDefinitionKeyLike(proccessDefKey).count();
		}else{
			l = taskService.createTaskQuery().processDefinitionKeyLike(proccessDefKey).count();
		}
		return new Long(l).intValue();
	}
	/**
	 * 根据服务分类获取待办
	 * @param retrieveKey 检索器key  1.22 把第一个参数由 服务分类 变成了 检索器key。服务分类过滤请放到 con条件中
	 * @param con 一些条件
	 * @param userId
	 * @return
	 */
	public int getTodoWorkTotalByST(String retrieveKey,Map con,String userId){
		...
		if(runtimeService==null){
			runtimeService = ApplicationContextManager.getContext().getBean(RuntimeService.class);
		}
		...
		if(con!=null&&con.get(WorkFlowService.IS_EXTERNAL_STORAGE_KEY)!=null){
			...
			if(isExternalStorage){
				return todoWorkService.countTodoWorkWithExternalStorage(retrieveKey, con, userId);
			}
		}
				...
				if(o!=null){
					if (field.getFieldType()==null||field.getFieldType().equals("STR")) {
						if(item.isIsLike()){//模糊查询
							mTaskQuery = (MTaskQueryImpl) mTaskQuery.processVariableValueLike(field.getFieldNo(), "%"+(String)o+"%");
						}else{
							mTaskQuery = (MTaskQueryImpl) mTaskQuery.processVariableValueEquals(field.getFieldNo(), (String)o);
						}	
					}
	 ...
	 * @return
	 */
	public Map<String,Integer> getTodoWorkTotalbyTypes(String[] serviceType,Map con,String userId){
		...
		if(serviceType==null){
			...
			for(DictDataInfo d:list){
				...
				map.put(d.getDictdataName(), this.getTodoWorkTotalByST(null,con, userId));
			}
			return map;
		}else{
	 ...
	 * @return
	 */
	public List<Map> getTodoWorkBySP(ServiceProduct sp,String userId, int offset,int limit){
		if(taskService==null){
			taskService = ApplicationContextManager.getContext().getBean(TaskService.class);
		}
		if(runtimeService==null){
		...
		ServiceAgreementService slas = ApplicationContextManager.getContext().getBean(ServiceAgreementService.class);	
...
//		ProcessDefinitionQuery pdq = repositoryService.createProcessDefinitionQuery();
		...
		List<Task> list;
		if(userId!=null&&productNo!=null){
			list = taskService.createTaskQuery().processVariableValueEquals(WorkFlowService.PRODUCTNO,productNo).taskCandidateOrAssigned(userId).processDefinitionKeyLike(proccessDefKey)
					.orderByDueDate().asc().listPage(offset, limit);
		}else if(userId!=null&&productNo==null){
			list = taskService.createTaskQuery().taskCandidateOrAssigned(userId).processDefinitionKeyLike(proccessDefKey)
					.orderByDueDate().asc().listPage(offset, limit);
		}else if(userId==null&&productNo!=null){			
			list = taskService.createTaskQuery().processVariableValueEquals(WorkFlowService.PRODUCTNO,productNo).processDefinitionKeyLike(proccessDefKey)
					.orderByDueDate().asc().listPage(offset, limit);
		}else{
			list = taskService.createTaskQuery().processDefinitionKeyLike(proccessDefKey)
					.orderByDueDate().asc().listPage(offset, limit);
		}
		ServiceAgreementService sams = ApplicationContextManager.getContext().getBean(ServiceAgreementService.class);
		for(Task t:list){		
			ProcessInstance pi = piq.processInstanceId(t.getProcessInstanceId()).singleResult();	
			String businessKey = pi.getBusinessKey();
			Map<String,Object> map = new HashMap<String,Object>();

			FormService formService = ApplicationContextManager.getContext().getBean(FormService.class);	
			
			map = formService.getFormDataByFkFromProcessVariable(sp.getFormId(), false,businessKey, pi.getId());
...
//						wfRuntimeService = ApplicationContextManager.getContext().getBean(WfRuntimeService.class);
//					}
//					wfRuntimeService.deleteProcessInstancesById(wfo, "no data in business form",t.getProcessInstanceId());
//				} catch (Exception e) {
//					// TODO Auto-generated catch block
//					e.printStackTrace();
//					continue;
 ...
 * @return
 */
	public List<Map> getTodoWorkByST(String retrieveKey,Map con,String userId,int offset,int limit){
/*		Execution e = runtimeService.createExecutionQuery().executionId("E22D9CF694D44E7D808840F3A37DE818").singleResult();
		runtimeService.activateProcessInstanceById(e.getId());
//		runtimeService.suspendProcessInstanceById(e.getId());
		boolean b = e.isSuspended();
		...
		if(con!=null&&con.get(WorkFlowService.IS_EXTERNAL_STORAGE_KEY)!=null){
			boolean isExternalStorage = (boolean) con.get(WorkFlowService.IS_EXTERNAL_STORAGE_KEY);
			if(isExternalStorage){
				return todoWorkService.getTodoWorkWithExternalStorage(retrieveKey, con, userId, offset, limit);
			}
		}
		MTaskQueryImpl mTaskQuery = new MTaskQueryImpl();
		...
		TaskRetrievalService taskRetrievalService = ApplicationContextManager.getContext().getBean(TaskRetrievalService.class);
		List<Task> list = taskRetrievalService.getSortableTasks(mTaskQuery,(Sortable)con.get("sortable"));
		ProcessInstanceQuery  piq = runtimeService.createProcessInstanceQuery();
		ServiceAgreementService sams = ApplicationContextManager.getContext().getBean(ServiceAgreementService.class);
		AccessControlService acc=new AccessControlServiceImpl();
...
//			Map variablesmap = t.getProcessVariables();
			ProcessInstance pi = piq.includeProcessVariables().processInstanceId(t.getProcessInstanceId()).singleResult();	
...
/*			if(variablesmap.get(WorkFlowService.FORMIDSTR)!=null){
				String formId = (String) variablesmap.get(WorkFlowService.FORMIDSTR);
				FormService fs = ApplicationContextManager.getContext().getBean(FormService.class);
			...
			if(variablesmap.get(WorkFlowService.PRODUCTNO)!=null){
				String productNo1 = (String) variablesmap.get(WorkFlowService.PRODUCTNO);
...
//					long interruptLimitl = 60000*interruptLimit;
				...
				map.putAll(uccService.getConcernStatusInfo(curUserId, t.getProcessInstanceId(), sp.getProductId()));		
			}else{
//				map.put("serviceTypeId", sp.getServiceTypeId());
				map.put("businessKey", businessKey);
				map.put("proinsid", t.getProcessInstanceId());
...
//		}
	 ...
	 * @param businesskey
	 * @param taskid
	 * @param clazzstr class or form name
	 */
...
/*			clazz = Class.forName(clazzstr);
			Dao dao = DaoFactory.create(clazz);
			Object o = clazz.newInstance();
			Method m = clazz.getMethod("setBusinessKey", String.class);
			m.invoke(o, businesskey);
			return dao.selectOne(o);*/
	 ...
	 * @return
	 */
	public Map<String,Integer> getWorkTotalBySTS(String[] serviceType,Map con,Timestamp starttime,Timestamp endtime){
		Map<String,Integer> map = new HashMap<String,Integer>();
		int total = 0;
		...
		if(serviceType==null){
			...
			List<DictDataInfo> list = daod.select(cond);
			for(DictDataInfo d:list){
				int num = this.getWorkTotalByST(d.getDictdataName(), con,starttime, endtime);
				total = total+num;
	 ...
	 * @return
	 */
	public int getWorkTotalBySP(ServiceProduct sp,Map con,Timestamp starttime,Timestamp endtime){
		if(historyService==null){
			historyService = ApplicationContextManager.getContext().getBean(HistoryService.class);
		}
		...
		return (int) historyService.createHistoricProcessInstanceQuery().variableValueEquals(WorkFlowService.PRIKEY, priority)
				.variableValueEquals(WorkFlowService.PRODUCTNO, sp.getProductNo()).startedAfter(starttime).startedBefore(endtime).count();
//		return (int) runtimeService.createProcessInstanceQuery().active().variableValueEquals("productNo", sp.getProductNo()).count();
	}
	 ...
	 * @return
	 */
	public int getWorkTotalByST(String serviceType,Map con,Timestamp starttime,Timestamp endtime){
//		int total = 0;
//		List<Map<String,Object>> list = new ArrayList<Map<String,Object>>();
		...
		HistoricProcessInstanceQuery hiq = historyService.createHistoricProcessInstanceQuery();
		if(starttime!=null&&endtime!=null){
			hiq = hiq.startedAfter(starttime).startedBefore(endtime);
		}
		if(serviceType!=null){
			hiq = hiq.variableValueEquals(WorkFlowService.SERVICETYPEKEY, serviceType);
		}
		if(priority!=null){
			hiq = hiq.variableValueEquals(WorkFlowService.PRIKEY, priority);
		}
		if(branchId!=null){
			hiq = hiq.variableValueEquals(WorkFlowService.APPLYBRANCHKEY, branchId);
		}
...
//				.variableValueEquals(WorkFlowService.APPLYBRANCHKEY, branchId).startedAfter(starttime).startedBefore(endtime).count();
//		if(sps==null){
//			sps = ApplicationContextManager.getContext().getBean(ServiceProductService.class);
//		}
...
//		return total;
	...
	public int getAllHisWorkCount(String serviceType,Map con,Timestamp starttime,Timestamp endtime){
		if(starttime==null||endtime==null){
			endtime = new Timestamp(System.currentTimeMillis());
			starttime = new Timestamp(System.currentTimeMillis()-2592000000l); //最近一个月
		}
		...
		if(serviceType==null&&con!=null&&con.get(WorkFlowService.SERVICETYPEKEY)!=null){
			serviceType = (String) con.get(WorkFlowService.SERVICETYPEKEY);
		}
		String priority = null;
		if(con!=null&&con.get(WorkFlowService.PRIKEY)!=null){
		...
		HistoricProcessInstanceQuery hiq = historyService.createHistoricProcessInstanceQuery().startedAfter(starttime).startedBefore(endtime);
		if(serviceType!=null){
			hiq = hiq.variableValueEquals(WorkFlowService.SERVICETYPEKEY, serviceType);
		}
		if(workTitle!=null){
			hiq = hiq.variableValueLike(WorkFlowService.WORKTITLEKEY, "%"+workTitle+"%");
		}
		if(priority!=null){
			hiq = hiq.variableValueEquals(WorkFlowService.PRIKEY, priority);
		}
	 ...
	 * @return
	 */
	public List<Map<String,Object>> getAllWorkIns(String serviceType,Map con,Timestamp starttime,Timestamp endtime,int offset,int limit){
//		List<Map<String,Object>> listr = new ArrayList<Map<String,Object>>();
		...
		/*这样查询的话 会是内存分页*/
//		if(offset<0||limit<0){
//			listhpi = hiq.includeProcessVariables().orderByProcessInstanceStartTime().desc().list();
//		}else{
//			listhpi = hiq.includeProcessVariables().orderByProcessInstanceStartTime().desc().listPage(offset, limit);
//		}
		
		if(offset<0||limit<0){
			listhpi = hiq.orderByProcessInstanceStartTime().desc().list();
		}else{
			listhpi = hiq.orderByProcessInstanceStartTime().desc().listPage(offset, limit);
		}
...
//			Date endtime1 = hpi.getEndTime();
			...
			Map variablesmap = fs.getFormDataByFkFromProcessVariable(null,false, businessKey, null);
			map.putAll(variablesmap);
/*			if(variablesmap.get(WorkFlowService.FORMIDSTR)!=null){
				String formId = (String) variablesmap.get(WorkFlowService.FORMIDSTR);
				FormService fs = ApplicationContextManager.getContext().getBean(FormService.class);
				map = fs.getFormDataByFk(formId, businessKey);
				map.put(WorkFlowService.FORMIDSTR, variablesmap.get(WorkFlowService.FORMIDSTR));	
			}*/
			if(variablesmap.get(WorkFlowService.SERVICETYPEKEY)!=null){
				map.put("serviceTypeId", variablesmap.get(WorkFlowService.SERVICETYPEKEY));
			}
			
/*			List<HistoricVariableInstance> variables = historyService.createHistoricVariableInstanceQuery().processInstanceId(PorcessInstanceId).list();
			...
			for(HistoricVariableInstance hvi:variables){
				if(hvi.getVariableName().equals(WorkFlowService.SERVICETYPEKEY)){
					if(hvi.getValue()!=null){
						map.put("serviceTypeId", hvi.getValue());			
					}				
				}
			}*/
			List<Task> listtask = taskService.createTaskQuery().processInstanceId(PorcessInstanceId).list();
			if(listtask.size()>0){
				...
				map.put("workStage",activiname.substring(0, activiname.length()-1));
			}
	 ...
	 * @return
	 */
	public List<Map<String,Object>> getWorkInsBySTS(String[] serviceType,Map con,Timestamp starttime,Timestamp endtime,int offset,int limit){
		List<Map<String,Object>> listr = new ArrayList<Map<String,Object>>();
		if(starttime==null||endtime==null){
			endtime = new Timestamp(System.currentTimeMillis());
			starttime = new Timestamp(System.currentTimeMillis()-2592000000l); //最近一个月
		...
		if(serviceType==null){
			...
			List<DictDataInfo> list = daod.select(cond);
			for(DictDataInfo d:list){
				listr.addAll(this.getWorkInsByST(d.getDictdataName(), con,starttime, endtime, offset, limit));
			}
			return listr;
		}else{
			for(String s:serviceType){
				listr.addAll(this.getWorkInsByST(s, con,starttime, endtime, offset, limit));
				return listr;
			}
		}
		return listr;
	 ...
	 * @return
	 */
	public List<Map<String,Object>> getWorkInsByST(String serviceType,Map con,Timestamp starttime,Timestamp endtime,int offset,int limit){
		if(historyService==null){
			historyService = ApplicationContextManager.getContext().getBean(HistoryService.class);
		}
		if(taskService==null){
		...
		if(workTitle!=null){
			hiq = hiq.variableValueEquals(WorkFlowService.WORKTITLEKEY, workTitle);
		}
		if(priority!=null){
			hiq = hiq.variableValueEquals(WorkFlowService.PRIKEY, priority);
		}
		...
		//如果外面查询包含流程变量 则分页为内存分页，数据多了 不止会不会有问题，所以现在外面查询是不包含变量，吧list查出来以后 再对list中的各个值查询流程变量
		List<HistoricProcessInstance> listhpi = hiq.orderByProcessInstanceStartTime().desc().listPage(offset, limit);
		for(HistoricProcessInstance hpi:listhpi){
			String businessKey = hpi.getBusinessKey();
			String PorcessInstanceId = hpi.getId();
			Map<String,Object> map = new HashMap<String,Object>();
//			ProcessInstance pi = piq.processInstanceId(PorcessInstanceId).singleResult();	
//			Map map = runtimeService.getVariables(t.getExecutionId());//获取流程变量
//			historyService.createHistoricProcessInstanceQuery().includeProcessVariables().processInstanceId(PorcessInstanceId).singleResult();
			...
			FormService fs = ApplicationContextManager.getContext().getBean(FormService.class);
			map = fs.getFormDataByFkFromProcessVariable(null, false,businessKey, null);
/*			List<HistoricVariableInstance> variables = historyService.createHistoricVariableInstanceQuery().processInstanceId(PorcessInstanceId).list();
			FormService formService = ApplicationContextManager.getContext().getBean(FormService.class);
			for(HistoricVariableInstance hvi:variables){
				if(hvi.getVariableName().equals(WorkFlowService.FORMIDSTR)){
...
//		List<ServiceProduct> listsp = sps.getSPByserviceType(serviceType);
//		for(ServiceProduct sp:listsp){
//			list.addAll(this.getWorkInsBySP(sp, con,starttime, endtime, offset, limit));
//		}
//		return list;
	}
	/**
	 ...
	 * @return
	 */
	public List<Map<String, Object>> getWorkInsBySP(ServiceProduct sp,Map con,Timestamp starttime,Timestamp endtime,int offset,int limit){
		if(historyService==null){
			historyService = ApplicationContextManager.getContext().getBean(HistoryService.class);
		}
		if(taskService==null){
...
//		ProcessInstanceQuery  piq = runtimeService.createProcessInstanceQuery();
		List<Map<String,Object>> listmap = new ArrayList<Map<String,Object>>();
//		List list = runtimeService.createProcessInstanceQuery().variableValueEquals(WorkFlowService.PRODUCTNO, sp.getProductNo()).list();
		List<HistoricProcessInstance> listhpi = historyService.createHistoricProcessInstanceQuery().variableValueEquals(WorkFlowService.PRIKEY, priority)
				.variableValueEquals(WorkFlowService.PRODUCTNO, sp.getProductNo()).startedAfter(starttime).startedBefore(endtime)
...
//			}
	 ...
	 * @return
	 */
	public int getWorkInHandleTotalBySP(ServiceProduct sp,Map con,Timestamp starttime,Timestamp endtime){
		if(historyService==null){
			historyService = ApplicationContextManager.getContext().getBean(HistoryService.class);
		}
		HistoricProcessInstanceQuery hpi = historyService.createHistoricProcessInstanceQuery().unfinished().startedAfter(starttime).startedBefore(endtime);
		if(sp!=null){
		...
		if(branchId!=null){
			hpi = hpi.variableValueEquals(WorkFlowService.APPLYBRANCHKEY, branchId);
		}
...
//		return (int) runtimeService.createProcessInstanceQuery().active().variableValueEquals("productNo", sp.getProductNo()).count();
	 ...
	 * @return
	 */
	public int getWorkClosedTotalBySP(ServiceProduct sp,Map con,Timestamp starttime,Timestamp endtime){
		if(historyService==null){
			historyService = ApplicationContextManager.getContext().getBean(HistoryService.class);
		}
		HistoricProcessInstanceQuery hpi = historyService.createHistoricProcessInstanceQuery().finished().startedAfter(starttime).startedBefore(endtime);
		if(sp!=null){
			hpi = hpi.variableValueEquals("productNo", sp.getProductNo());
		}
		String priority = null;
	 ...
	 * @return
	 */
	public List<HistoricProcessInstance> getProcessInstancesByProcessVariable(Map variable,Timestamp starttime,Timestamp endtime){
		if(historyService==null){
			historyService = ApplicationContextManager.getContext().getBean(HistoryService.class);
		}
		...
		Boolean needFinished = null;
		if(variable!=null&&variable.get(WorkFlowService.NEEDFINISHED)!=null){
		...
		Boolean needUnFinished = null;
		if(variable!=null&&variable.get(WorkFlowService.NEEDUNFINISHED)!=null){
			needUnFinished = (Boolean) variable.get(WorkFlowService.NEEDUNFINISHED);
			if(needUnFinished){
				hq = hq.unfinished();
			}
		}
	...
	//获取实例流程变量
	public List<HistoricVariableInstance> getHistoricVariableByInsId(String processInstanceId){
		if(historyService==null){
			historyService = ApplicationContextManager.getContext().getBean(HistoryService.class);
		}
	 ...
	 * @return
	 */
	public Object getHistoricVariableByIdKey(String processInstanceId,String key){


		List<HistoricVariableInstance> list = this.getHistoricVariableByInsId(processInstanceId);
	 ...
	 * 
	 * @param branchKey 部门流程变量的key
	 * @param starttime
	 * @param endtime
	 * @return
	 */
	public Map<String,Integer> getWorkBranchStatistic(String branchKey,Timestamp starttime,Timestamp endtime){
		Map<String,Integer> map = new HashMap<String,Integer>();
		...
		List<HistoricProcessInstance> list = this.getProcessInstancesByProcessVariable(null,starttime,endtime);
		for(HistoricProcessInstance hp:list){
			...
			if(o1!=null&&s!=null&&branches.get(s)!=null){
				...
				map.put(WorkFlowService.TOTAL, map.get(WorkFlowService.TOTAL)+1);
			}else if(o1!=null&&s!=null&&branches.get(s)==null&&orgService.queryByPK(s)!=null){	
				...
				map.put(WorkFlowService.TOTAL, map.get(WorkFlowService.TOTAL)+1);
			}else if(o1!=null&&(s==null||orgService.queryByPK(s)==null)){
	...
	public WfProcessDefinition getBindProcessByModuleId(String moduleId){
		if(wfManagerService==null){
			wfManagerService = ApplicationContextManager.getContext().getBean(WfManagerService.class);
		}
		WfProcessDefinition wfProcessDefinition = null;
		try {
			wfProcessDefinition = wfManagerService.getBindProcessByModuleId(moduleId);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
...
//		work = daows.selectOne(work);	
		HistoricProcessInstance hpi = historyService.createHistoricProcessInstanceQuery().processInstanceBusinessKey(wworkId).singleResult();
		if(hpi==null){
			return null;
		}
		String PorcessInstanceId = hpi.getId();
...
//			}
			...
			if(hvi.getVariableName().equals(WorkFlowService.PRODUCTNO)){
				ServiceProduct sp = sps.getServiceProductByNo((String) hvi.getValue());
				map.put("serviceTypeId", sp.getServiceTypeId());
			}
			if(hvi.getVariableName().equals(WorkFlowService.HAPPENEDTIMEL)){
				Object o = hvi.getValue();
				if(o instanceof String){
					map.put("happenDate", new Timestamp(Long.valueOf( (String) hvi.getValue())));
				}else if(o instanceof Long){
	...
	public boolean hasUnfinishedServiceProductWork(String productNo){
		...
		List list = this.getProcessInstancesByProcessVariable(variable, null, null);
