...
//				taskService.
	...
	public List<EmailTask> informUserConcernedConfigTask(ActivitiEntityEvent e) throws Exception{		
		taskService = ApplicationContextManager.getContext().getBean(TaskService.class);
		runtimService = ApplicationContextManager.getContext().getBean(RuntimeService.class);
		productService = ApplicationContextManager.getContext().getBean(ServiceProductService.class);
		
		List<EmailTask> emailTasks = new ArrayList<EmailTask>();
		Dao<UserConcernedConfig> daoucc = DaoFactory.create(UserConcernedConfig.class);
		List<UserConcernedConfig> uccsdel = new ArrayList<UserConcernedConfig>();//关注的流程实例结束了 ，id不存在了时 需要删除配置
		TaskEntity entity = (TaskEntity) e.getEntity();
		String taskId = entity.getId();
		...
		userConcernedConfigService = ApplicationContextManager.getContext().getBean(UserConcernedConfigService.class);
		List<UserConcernedConfig> uccs = userConcernedConfigService.getUserConcernedConfig(ucc);
		...
		for(UserConcernedConfig ucc1:uccs){
			...
			//表示有人关注了这个工单,且不分环节		
			boolean isConcernedWork = ucc1.getProcessInsId()!=null&&ucc1.getProcessInsId().equals(proInsId)&&ucc1.getTaskDefId()==null;
			//这是有人关注了 流程产品了
			boolean isConcernedSP = ucc1.getProcessInsId()==null&&ucc1.getTaskDefId()==null;	
			//有关注产品一个环节
			boolean isConcernedActivity = ucc1.getProcessInsId()==null&&ucc1.getTaskDefId()!=null&&ucc1.getTaskDefId()==taskDefId;
			if(isConcernedWork||isConcernedSP||isConcernedActivity){			
				ProcessInstance p = runtimService.createProcessInstanceQuery().includeProcessVariables().processInstanceId(proInsId).singleResult();
				//实例不存在了 删除配置
				if(p==null){
					uccsdel.add(ucc1);
				}else{	
...
/*		for(EmailTask t:emailTasks){
	...
	public void informActivityConfigComment(ActivitiEntityEvent e) throws Exception{/*
		List<EmailTask> emailTasks = new ArrayList<EmailTask>();
		...
		activityConfigService = ApplicationContextManager.getContext().getBean(ActivityConfigService.class);
		ActivityConfig ac = activityConfigService.getActivityConfigById(task.getTaskDefinitionKey());
		if(ac!=null){
			//是否有此事件配置。有时候我只要通知任务事件 不需要通知评论事件
			if(ac.getAction()==null){
				return;
			}
			String[] events = ac.getAction().split(",");
			for(String event:events){
				if(event.equals(InformService.EVENT_TYPE_COMMENT)){
					break;
				}
				return;
			}
...
//			List<IdentityLink> identity = taskService.getIdentityLinksForTask(taskId);
			...
			Map<String,String> contentMap = new HashMap<String,String>();
			contentMap.put(InformService.WORKSTATGEKEY, message);
			contentMap.put(InformService.CATEGORY_KEY,"嗯 这里是什么意思？");
			contentMap.put(InformService.STR_TEMPLATE_KEY, InformService.TASK_INFORM_EMAIL_TEMPLATE);
			if(identity!=null&&identity.size()>0){
				for(IdentityLink i:identity){
	...
	*/}
	//用户关注设置的评论事件通知
	public List<EmailTask> informUserConcernedConfigComment(ActivitiEntityEvent e) throws Exception{
		taskService = ApplicationContextManager.getContext().getBean(TaskService.class);
		runtimService = ApplicationContextManager.getContext().getBean(RuntimeService.class);
		productService = ApplicationContextManager.getContext().getBean(ServiceProductService.class);
		historyService = ApplicationContextManager.getContext().getBean(HistoryService.class);
		...
		CommentEntity entity = (CommentEntity) e.getEntity();
		String taskId = entity.getTaskId();
		HistoricTaskInstance task = historyService.createHistoricTaskInstanceQuery().includeProcessVariables().taskId(taskId).singleResult();
...
//		String message = entity.getMessage();
		...
		String productNo = (String) task.getProcessVariables().get(WorkFlowService.PRODUCTNOKEY);
		ServiceProduct sp = productService.getServiceProductByNo(productNo);
		...
		for(UserConcernedConfig ucc1:uccs){
			...
			boolean isConcernedSP = ucc1.getProcessInsId()==null&&ucc1.getTaskDefId()==null;
			//有关注产品一个环节
			boolean isConcernedActivity = ucc1.getProcessInsId()==null&&ucc1.getTaskDefId().equals(taskDef);
			if(isConcernedWork||isConcernedSP||isConcernedActivity){
				Map<String,String> contentMap = new HashMap<String,String>();
				//此处还没调试，工单结束环节。此时应该没有实例了
				if(proInsId!=null){
					WorkFlowService wfs = ApplicationContextManager.getContext().getBean(WorkFlowService.class);
					String workTitle = (String) wfs.getHistoricVariableByIdKey(proInsId,WorkFlowService.WORKTITLEKEY);
					contentMap.put(WorkFlowService.WORKTITLEKEY, workTitle);
					ProcessInstance p = runtimService.createProcessInstanceQuery().processInstanceId(proInsId).singleResult();
